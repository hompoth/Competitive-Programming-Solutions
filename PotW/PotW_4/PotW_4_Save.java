	...
	static int[][] sumRectangle(int[][] arr){
		...
		for(int i = 0; i < M; ++i){
			...
			for(int j = 0, k=0, sum=0; j < N; ++j){
				...
				else tmp=numArray[i][j]-min-minArray[j]+numArray[k][minPos[j]];
				System.out.println(tmp+" "+min+" "+minArray[j]+" "+numArray[k][minPos[j]]+" : "+i+" "+j+" "+minPos[j]+" "+k);
				if(tmp>max) max=tmp;
				if(numArray[i][j]>max) max=numArray[i][j];
				if(min>numArray[i][j]) min=numArray[k=i][j];
				if(minArray[j]>numArray[i][j]) minArray[minPos[j]=j]=numArray[i][j];
			}
		}
		System.out.println("\n"+max+"\n");
		return numArray;
